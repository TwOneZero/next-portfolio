...
const ProjectItem = ({ data }) => {
  ...
  return (
    <div className='m-2 p-4  dark:bg-slate-700 rounded-md transition duration-300 transform border border-gray-300 hover:scale-110 hover:shadow-lg dark:hover:shadow-gray-400 dark:border-gray-200/50'>
      ...
      <h1 className='text-xl font-bold my-3'>{projectTitle}</h1>
      ...
      <h3 className='mt-4 text-ms'>작업 기간 : {getProjectPeriod(srt_d, end_d)}일</h3>

      <a className=' text-cyan-900 hover:text-cyan-600 hover:transition-colors' href={github_url.url} target='_blank'>Github</a>
